  ...
  export const ModalContentSection = ({ project, accentText }: ModalContentSectionProps) => (
    <div className="lg:col-span-2 flex flex-col gap-5 sm:gap-7 lg:h-full">
      ...
      {project.images && project.images.length > 0 && (
        <div className="shrink-0 hidden md:block">
          <h3 className={`text-xl sm:text-2xl font-bold mb-3 ${accentText} border-b border-gray-200 dark:border-gray-700 pb-2`}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-4">
            {project.images.slice(0, 6).map((img, i) => (
              <img
                ...
                src={`${img.url}`}
                alt={img.alt}
                className="w-full h-32  rounded-lg shadow-md border border-gray-200 dark:border-gray-700 hover:scale-[1.02] transition-transform duration-300 cursor-zoom-in"
              />
            ))}
          </div>
        </div>
